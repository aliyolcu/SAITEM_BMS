...
/* USER CODE BEGIN (3) */
	...
	int nSent = 0, nRead = 0, nTopFound = 0; //nTopFound = bqlar birbirine bağlanıyor hangisini slave hangisini master olması gerektiğini bulmak için kullanılıyor.
	int nDev_ID, nGrp_ID;
	BYTE  bFrame[132];
	uint32  wTemp = 0;
	...
	nSent = WriteReg(0, 81, 0x38, 1, FRMWRT_ALL_NR); // clear fault flags in the system status register //bakacam
	...
	nRead = ReadReg(nDev_ID, 82, &wTemp, 2, 0); // 0ms timeout //bAK ALI
